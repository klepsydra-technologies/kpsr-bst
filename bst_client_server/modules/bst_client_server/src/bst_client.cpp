...
kpsr::bst::BstClient::BstClient(
    ...
    , _clientStateMachine(environment,
                          bstClientMiddlewareProvider,
                          clientStateMachinePublisher,
                          clientStateMachineExtSubscriber,
...
{
    _clientStateMachineSubscriber
        ->registerListener("BST_CLIENT", _clientStateMachineListener.cacheListenerFunction);
    ...
    if (heartbeatPeriod > 0) {
        ...
        _bstClientMiddlewareProvider->getScheduler()->startScheduledTask("BST_CLIENT_HEARTBEAT",
                                                                         heartbeatPeriod,
                                                                         true,
                                                                         task);
    }
    ...
    if (controlPeriod > 0) {
        ...
        _bstClientMiddlewareProvider->getScheduler()->startScheduledTask("BST_CLIENT_CONTROL_BEAT",
                                                                         controlPeriod,
                                                                         true,
                                                                         task);
    }
...
bool kpsr::bst::BstClient::acquirePayloadControl()
{
    ...
    if (*_clientStateMachineListener.getLastReceivedEvent().get() ==
        "bstClientStateMachine:ready") {
...
bool kpsr::bst::BstClient::releasePayloadControl()
{
    if (*_clientStateMachineListener.getLastReceivedEvent().get() ==
        ...
        *_clientStateMachineListener.getLastReceivedEvent().get() ==
            "bstClientStateMachine:landed") {
