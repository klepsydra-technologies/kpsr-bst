...
TEST(ZMQStgCoreTests, PubSubTest)
{
    ...
    kpsr::Publisher<SystemInitialize_t> *toZMQPublisher =
        toZMQMiddlewareProvider.getVoidCasterToMiddlewareChannel<SystemInitialize_t>(topic, 0);
    ...
    kpsr::zmq_mdlw::FromZmqChannel<std::vector<unsigned char>> *fromZMQProvider =
        fromZmqMiddlewareProvider.getVoidCasterFromMiddlewareChannel<SystemInitialize_t>(subscriber,
                                                                                         10);
    //spdlog::info("Creating services.");
    SimpleWriteService writeService(nullptr, toZMQPublisher);

    kpsr::EventEmitterMiddlewareProvider<SystemInitialize_t> eventEmitter(nullptr,
                                                                          "bst_system_init",
                                                                          8,
                                                                          nullptr,
                                                                          nullptr);
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (cacheListener.counter > 0) {
            ASSERT_EQ(writeService.event.vehicle_type,
                      cacheListener.getLastReceivedEvent()->vehicle_type);
        }
    }
