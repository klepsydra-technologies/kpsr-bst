...
double kpsr::bst::LatLongUtils::CoordinatesToAngle(double latitude1,
...
double kpsr::bst::LatLongUtils::CoordinatesToMeters(double latitude1,
                                                    double longitude1,
                                                    double latitude2,
                                                    double longitude2)
{
    ...
    return earthDiameterMeters *
           std::asin(sqrt((x * x) + (std::cos(latitude1) * std::cos(latitude2) * y * y)));
}
