...
void kpsr::bst::Bst2KpsrAdaptorService::start()
{
    ...
    std::function<void(Bst2KpsrInternalMessage)> bst2KpsrAdaptorListener =
        std::bind(&Bst2KpsrAdaptorService::onMessageReceived, this, std::placeholders::_1);
    Bst2KpsrMiddlewareProvider::getBst2KpsrInternalMessageSubscriber()
        ->registerListener("Bst2KpsrAdaptorService", bst2KpsrAdaptorListener);
}

void kpsr::bst::Bst2KpsrAdaptorService::stop()
{
    Bst2KpsrMiddlewareProvider::getBst2KpsrInternalMessageSubscriber()->removeListener(
        "Bst2KpsrAdaptorService");
...
void kpsr::bst::Bst2KpsrAdaptorService::onMessageReceived(const Bst2KpsrInternalMessage &message)
{
    ...
    case COMMS_FUNCTION::RECEIVE:
        ...
        _bst2KpsrBroadcaster.receive(message.type,
                                     message.data,
                                     message.size,
                                     message.receivedParameter);
        break;
    case COMMS_FUNCTION::RECEIVE_COMMAND:
        spdlog::debug("{} RECEIVE_COMMAND", __PRETTY_FUNCTION__);
        _bst2KpsrBroadcaster.receiveCommand(message.type,
                                            message.data,
                                            message.size,
                                            message.receivedParameter);
        break;
    case COMMS_FUNCTION::RECEIVE_REPLY:
        spdlog::debug("{} RECEIVE_REPLY: type={}, data={}, ack: {}",
                      __PRETTY_FUNCTION__,
                      (int) message.type,
                      (int) message.data[0],
                      (message.ack ? "ACK" : "NACK"));
        _bst2KpsrBroadcaster.receiveReply(message.type,
                                          message.data,
                                          message.size,
                                          message.ack,
                                          message.receivedParameter);
