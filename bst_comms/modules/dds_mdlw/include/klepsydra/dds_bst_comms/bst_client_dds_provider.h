...
public:
     ...
     */
    BstClientDDSProvider(Container * container,
    ...
    void start() override {
        {
            kpsr::Publisher<kpsr::bst::BstReplyMessage> * publisher = _eventloopProvider.template getPublisher<kpsr::bst::BstReplyMessage>(EVENT_NAME_BST_REPLY, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_BST_REPLY, _bstReplyDataReader, true, publisher);
        }

        {
            kpsr::Publisher<kpsr::geometry::PoseEventData> * publisher = _eventloopProvider.template getPublisher<kpsr::geometry::PoseEventData>(EVENT_NAME_POSE_EVENT, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_POSE_EVENT, _poseDataReader, true, publisher);
        }
        ...
        {
            kpsr::Publisher<Sensors_t> * publisher = _eventloopProvider.template getPublisher<Sensors_t>(EVENT_NAME_SENSOR, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_SENSOR, _sensorsReader, true, publisher);
        }

        {
            kpsr::Publisher<CalibrateSensor_t> * publisher = _eventloopProvider.template getPublisher<CalibrateSensor_t>(EVENT_NAME_CALIBRATE, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_CALIBRATE, _calibrateSensorReader, true, publisher);
        }

        {
            kpsr::Publisher<Command_t> * publisher = _eventloopProvider.template getPublisher<Command_t>(EVENT_NAME_CONTROL_CMD, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_CONTROL_CMD, _commandReader, true, publisher);
        }

        {
            kpsr::Publisher<PID_t> * publisher = _eventloopProvider.template getPublisher<PID_t>(EVENT_NAME_CONTROL_PID, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_CONTROL_PID, _pidReader, true, publisher);
        }

        {
            kpsr::Publisher<uint8_t> * publisher = _eventloopProvider.template getPublisher<uint8_t>(EVENT_NAME_SYSTEM, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_SYSTEM, _systemReader, true, publisher);
        }

        {
            kpsr::Publisher<TelemetrySystem_t> * publisher = _eventloopProvider.template getPublisher<TelemetrySystem_t>(EVENT_NAME_TELEMETRY_SYS, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_TELEMETRY_SYS, _telemetrySystemReader, true, publisher);
        }

        {
            kpsr::Publisher<TelemetryPressure_t> * publisher = _eventloopProvider.template getPublisher<TelemetryPressure_t>(EVENT_NAME_TELEMETRY_PRES, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_TELEMETRY_PRES, _telemetryPressureReader, true, publisher);
        }

        {
            kpsr::Publisher<::bst::comms::TelemetryControl_t> * publisher = _eventloopProvider.template getPublisher<::bst::comms::TelemetryControl_t>(EVENT_NAME_TELEMETRY_CTRL, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_TELEMETRY_CTRL, _telemetryControlReader, true, publisher);
        }

        {
            kpsr::Publisher<gcs::TelemetryGCS_t> * publisher = _eventloopProvider.template getPublisher<gcs::TelemetryGCS_t>(EVENT_NAME_TELEMETRY_GCS, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_TELEMETRY_GCS, _telemetryGCSReader, true, publisher);
        }

        {
            kpsr::Publisher<PayloadControl_t> * publisher = _eventloopProvider.template getPublisher<PayloadControl_t>(EVENT_NAME_PAYLOAD_CONTROL, 0, nullptr, nullptr);
            _fromDDSMiddlewareProvider.registerToTopic(EVENT_NAME_PAYLOAD_CONTROL, _payloadControlReader, true, publisher);
        }
    }
    ...
    void stop() override {
        ...
        _fromDDSMiddlewareProvider.unregisterFromTopic(EVENT_NAME_TELEMETRY_SYS, _telemetrySystemReader);

        _fromDDSMiddlewareProvider.unregisterFromTopic(EVENT_NAME_TELEMETRY_PRES, _telemetryPressureReader);

        _fromDDSMiddlewareProvider.unregisterFromTopic(EVENT_NAME_TELEMETRY_CTRL, _telemetryControlReader);

        _fromDDSMiddlewareProvider.unregisterFromTopic(EVENT_NAME_TELEMETRY_GCS, _telemetryGCSReader);

        _fromDDSMiddlewareProvider.unregisterFromTopic(EVENT_NAME_PAYLOAD_CONTROL, _payloadControlReader);
    }
    ...
    kpsr::Publisher<kpsr::bst::BstRequestMessage> * getBstRequestMessagePublisher() {
        return _toDDSMiddlewareProvider.getToMiddlewareChannel<kpsr::bst::BstRequestMessage, kpsr_dds_bst::BstRequestData>("dds_bst_request", 0, nullptr, _bstRequestDataWriter);
    }
    ...
    kpsr::Publisher<WaypointCommandMessage> * getBstWaypointCommandMessagePublisher() {
        return _toDDSMiddlewareProvider.getToMiddlewareChannel<kpsr::bst::WaypointCommandMessage, kpsr_dds_bst::WaypointCommandData>(EVENT_NAME_BST_WP_CMD, 0, nullptr, _bstWaypointCommandDataWriter);
    }
